...
(function ($) {
	...
	var $caption = $('<div />', { 'id': 'custom-trigger' });
	$caption
		.html('<h3 class="img-caption"></h3><small class="img-desc"></small>')
		.appendTo($('body'));

	// Initialize Fluidbox
	var fluid = $('a').fluidbox()
		.on('openend.fluidbox', function () {
			var $img = $(this).find('img');
			var name = $img.data("name");
			var description = $img.data("desc");
			$('#custom-trigger')
				.addClass('visible')
				.find('.img-caption')
				.html(name);
		...
		.on('closestart.fluidbox', function () {
			$('#custom-trigger').removeClass('visible');
		});
